...
def order_tris(tri):
    ...
    :param tri: Triangulation (n_v x 3) np.int64 array
...
def make_y(x, Lgrid_xy):
    ...
    :param x: Cell centroids (n_c x 2) np.float64 array
    :param Lgrid_xy: (9 x 2) array defining the displacement vectors for each of the 9 images of the tiling
    :return: Tiled set of coordinates (9n_c x 2) np.float64 array
    """
    n_c = x.shape[0]
    y = np.empty((n_c * 9, x.shape[1]),dtype=np.float64)
    for k in range(9):
...
def tri_mat_call(mat, tri, direc=-1):
    ...
    nv = tri.shape[0]
    tmat = np.empty((nv, 3),dtype=np.float64)
...
def circumcenter(C, L):
    ...
    :param C: Cell centroids for each triangle in triangulation (n_c x 3 x 2) np.float64 array
    :param L: Domain size (np.float64)
    :return: Circumcentres/vertex-positions (n_v x 2) np.float64 array
    """
    ...
    vs[:, 0], vs[:, 1] = ux, uy
    vs = np.mod(vs + disp.T, L).astype(np.float64)
    return vs
...
def get_neighbours(tri):
    ...
    :return: (n_v x 3) np.int64 array, storing the three neighbouring triangles. Values correspond to the row numbers of tri
    """
    n_v = tri.shape[0]
    neigh = np.ones_like(tri, dtype=np.int64) * -1
...
def tri_angles_periodic(x, tri, L):
    ...
    C = np.empty((tri.shape[0], 3, 2),dtype=np.float64)
